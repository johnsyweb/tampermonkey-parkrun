...
(() => {
  ...
  // src/p-index.user.js
  var require_p_index_user = __commonJS({
    "src/p-index.user.js"(exports, module) {
      (function() {
        "use strict";
        const {
          calculatePIndex,
        ...
        function buildContributingEvents(groupedEvents, pIndex) {
          return groupedEvents.slice(0, pIndex).map(([eventName, events]) => {
          ...
          }).sort((a, b) => a.pIndexDateForSorting - b.pIndexDateForSorting).map(
            (event) => event.eventName + " (" + event.eventCount + "): " + event.firstDate + " (#" + event.firstEventNumber + ") - " + event.pIndexDate + " (#" + event.pIndexEventNumber + ")"
          );
        }
      })();
    }
  });
  require_p_index_user();
})();
